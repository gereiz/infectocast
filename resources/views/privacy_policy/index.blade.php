...
@section('content')
    ...
    <x-page-title title="Política de Privvacidade" pagetitle="Políticas" />
        ...
        <div class="card-body">
            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-2">
                ...
                <div class="ltr:md:text-end rtl:md:text-start">
                    <a type="button" href="{{url('addPolicy')}}" class="btn-add">
                ...
                <table class="w-full whitespace-nowrap" id="customerTable">
                    ...
                    <tbody class="list form-check-all">
                        @foreach ($policies as $policy)
                            <tr>
                                ...
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 id"
                                    style="display:none;"><a href="javascript:void(0);"
                                        class="fw-medium link-primary id">{{$policy->id}}</a></td>
                                ...
                                <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500">
                                    <div class="flex gap-2">
                                        <div class="edit">
                                            <a href="{{url('addPolicy/'.$policy->id)}}"
                                        ...
                                        <div class="remove">
                                            <button data-modal-target="{{'deleteModal/'.$policy->id}}" id="delete-record" class="py-1 text-xs text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 dark:ring-custom-400/20 remove-item-btn">
                                                Excluir
                                            </button>
                                        </div>
                                    </div>
                            ...
                            {{-- Modal Delete --}}
                            <div id={{'deleteModal/'.$policy->id}} modal-center
                                class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
                                <div class="w-screen md:w-[25rem] bg-white shadow rounded-md dark:bg-zink-600">
                                    <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto px-6 py-8">
                                        <form method="POST" action="{{urL('deletePolicy')}}">
                                            @csrf
                                            <div class="mb-3" id="modal-id" style="display: none;">
                                                <label for="id_policy" class="inline-block mb-2 text-base font-medium">ID</label>
                                                <input type="text" id="id_policy" name="id_policy" value="{{$policy->id}}"
                                                    class="input-text"
                                                    placeholder="ID" readonly="">
                                            </div>
                                            <div class="float-right">
                                                <button data-modal-close={{'deleteModal/'.$policy->id}} id="close-removeNotesModal"
                                            ...
                                            <div class="mt-5 text-center">
                                                ...
                                                <div class="flex justify-center gap-2 mt-6">
                                                    <button type="button" data-modal-close={{'deleteModal/'.$policy->id}}
